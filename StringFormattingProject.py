...
for i in range(len(daily_transactions_split)):
  x = daily_transactions_split[i]
  temp = []
  for j in range(len(x)):
    temp.append(x[j].strip())
...
for i in range(len(transactions_clean)):
  x = transactions_clean[i]
  for j in range(len(x)):
    if j == 0:
      customers.append(x[j])
    elif j == 1:
      sales.append(x[j])
    elif j == 2:
      thread_sold.append(x[j])
    else:
      continue
...
for i in range(len(sales)):
  noformat = []
  noformat.append(sales[i].strip('$'))
  for j in range(len(noformat)):
    total_sales += float(noformat[j])


thread_sold_split = []
for i in range(len(thread_sold)):
  x = []
  if thread_sold[i].count('&') == 0:
...
def color_count(color):
  ...
  for i in range(len(thread_sold_split)):
    if thread_sold_split[i] == color:
      ...
      continue
  return "{} threads of {} were sold today".format(color_count, color)
